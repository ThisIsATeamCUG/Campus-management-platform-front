<template>
  <div id="Login">
    ...
    <div class="Login">
      <form class="LoginBd">
        <div class="user">
          <input type="text" id="user" name="user" v-model="userName" placeholder="请输入账号或邮箱" maxlength="16">
        </div>
        <div class="password">
...
<script>
  ...
  export default {
    ...
    methods: {
      reggo() {
        this.$http
          .post('http://fleamarket.cn.utools.club/user/update?email='+this.email+'&passWord='+this.passWord,{
            code:this.code
          })
          .then(response=>{
      ...
      verifyClick(verify) {
        ...
        if (this.email!='') {
          ...
          this.$http
           .post('http://fleamarket.cn.utools.club/user/getCode',prom)
            .then(response=>{
                if(response.data.data==true){
      ...
      updataInfo() {
        ...
        this.$http
          .post('http://fleamarket.cn.utools.club/user/login', prom)
          .then(response => {
